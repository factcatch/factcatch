...
def getSourceDesc():
    domains = db.session.query(GoogleResult.domain, func.count(GoogleResult.domain)).group_by(GoogleResult.domain).all()
...
def getSourcesWithClaim():
    ...
    for domain in domains:
        ...
        claimsId = GoogleResult.query.with_entities(GoogleResult.claim_id).filter_by(domain=domain[0]).all()
        # sourceClaim = []
        # sourceClaim.append(domain[0])
        # claims = []
        if len(claimsId) < 10:
        ...
        for index,claim in enumerate(claimsId):
            ...
            if index < max_cell:
                ...
                sourceName = getEmptyString(spaces) #domain[0] + " " #str(int(index/max_cell))
            results.append({
                "source_id" : 0,
                "source" : sourceName,
                "claim" : claim[0],
                "index" : index%max_cell,
                "credibility_claim" : max_cell-index if max_cell-index >= 0 else 0 #credibilitySource
            })
